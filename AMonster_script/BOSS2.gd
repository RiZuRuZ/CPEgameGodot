...
# BOSS SETTINGS
# ===============================
@export var SPEED: float = 35.0          # ปรับให้ช้าลงสมเป็นบอส
@export var STOP_RADIUS: float = 28.0    
...
# STATS (BOSS)
# ===============================
@export var health: int = 600            # ⭐ เลือดเยอะแบบบอส
@export var atk1dmg : int = 40           # ⭐ ปรับดาเมจขึ้นเล็กน้อย
@export var atk2dmg : int = 40
@export var bodydmg : int = 25

var PreHealth = 0
var damaged := false
var enraged := false     # ⭐ ตัวแปรเช็คสถานะ Rage
...
# ===============================
# SUMMON SYSTEM (BOSS FEATURE)
# ===============================
@export var summon_scenes: Array[PackedScene]   # ลาก Scene มอนลูกน้องมาใส่ตรงนี้
@export var summon_cooldown: float = 8.0
@export var max_minions: int = 5
@export var summon_radius: float = 50.0
...
func _ready() -> void:
	randomize()
	add_to_group("EnemyBody") # จัด Group ให้เหมือนบอสตัวเก่า
	
	# ⭐ Setup Boss
	scale = Vector2(2, 2) # ขยายตัวใหญ่ขึ้น
	PreHealth = health
	
	# Disable collisions setup
...
func _physics_process(delta: float) -> void:
	...
	if health <= 0 and not death:
		...
		can_move = false
		is_hurt = false
		summon_timer = 9999 # หยุดเสกของ
		$Area2D.set_deferred("disable", false) # ตามโค้ดต้นฉบับ
		play_anim("death")
		return
	...
	# --- RAGE PHASE CHECK (BOSS FEATURE) ---
	if not enraged and health <= 300: # เลือดต่ำกว่า 40%
		_enter_rage()
...
func attack_coroutine() -> void:
	...
	# ⭐ คงรูปแบบการโจมตีเดิมตามคำสั่ง (เช็คแกน Y)
	if to_player.y < -2:
		play_anim("attack1")
	else:
		play_anim("attack2")
...
func _enter_rage():
	enraged = true
	SPEED *= 2              # เพิ่มความเร็ว
	atk1dmg += 10             # เพิ่มดาเมจ
	atk2dmg += 10
	summon_timer = 1.0        # เริ่มเสกตัวลูกเร็วขึ้น
	
	# ใส่ Effect Rage ตรงนี้ได้ (เช่น เปลี่ยนสีตัว)
	modulate = Color(1.5, 0.5, 0.5)
	print(">>> BOSS ENRAGED MODE <<<")
	
func _try_summon():
	# ไม่มีการเช็ค max_minions แล้ว (เสกไม่อั้น)
...
func drop_item():
	...
	dropA.global_position = global_position # ดรอปตรงจุดตาย
...
func show_damage_to_player(amount: int, target: Node2D):
	...
	# ตั้งตำแหน่งให้ขึ้นบนหัวผู้เล่น (target)
