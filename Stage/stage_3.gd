...
@export var Moredmg :int =6
@export var Morespd :int = 6
@export var Morehealt :int =6

func _ready():
	# --- เก็บข้อมูล Stage → Waves --
	stages = {
		3: {
			1: [ [WEREWOLF, 2], [WEREBARE, 3], [SLIME,4] ],
			2: [ [ARMOREDSKELETON, 2],[ARCHERSKELETON,3] ,[ARMOREDORC,2] ],
			3: [ [GREATSWORDSKELETON, 2],[WEREBARE,1], [WEREWOLF,2],[ORCRIDER,1], [SKELETON,5] ],
			4: [ [ELITEORC,1] ,[ARMOREDSKELETON, 2], [ORC,4] , [SLIME,3]],
			5: [ [BOSS3,1] ,[WEREWOLF, 3], [WEREBARE, 3]]
		}
	}
...
func _physics_process(delta: float) -> void:
	...
	if player.health <= 0:
		...
		$"/root/Wave/CanvasLayer/time".visible = false
	if monster == 0 and current_stage == 3 and current_wave > 5:
